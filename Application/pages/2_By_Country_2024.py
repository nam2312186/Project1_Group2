...
#     # ============================================================
#     # 1) Metric sửa: số bài hát = số bài duy nhất; Popularity gắn với bài
#     #    -> dùng trung bình theo mỗi bài rồi lấy mean (tránh bias do trùng ngày)
#     # ============================================================
...
#     c4.metric("🔥 Độ phổ biến TB (trung bình theo bài)", None if pd.isna(pop_metric) else round(pop_metric, 1))


#     # =========================
#     # 3️⃣ Xu hướng thể loại (bản rút gọn – không median)
#     # =========================
...
#                # 1️⃣ Gộp theo bài – mỗi bài chỉ tính 1 lần
#         song_level = (
...
#         # 🚫 Bỏ toàn bộ thể loại "unknown"
#         remove_unknown = ["unknown", "unknow", "n/a", "none", "undefined", ""]
#         song_level = song_level[~song_level["main_genre"].str.lower().isin(remove_unknown)]

#         # 2️⃣ Lọc thể loại có đủ số bài tối thiểu
#         min_tracks = st.slider("🔢 Lọc thể loại có ít nhất bao nhiêu bài", 1, 20, 3, key=f"min_tracks_{feature}")
...
#         # 3️⃣ Tính trung bình theo thể loại
#         genre_agg = (
...
#     #    - Sửa trục: x = số track, y = nghệ sĩ (đã tách collab)
#     #    - Thêm Explicit ratio khi xem Top Bài hát
#     # ============================================================
...
#             x="popularity",
#             y="song",
#             color="popularity",                      # 🎯 màu theo độ nổi
#             color_continuous_scale="Greens",        # thang xanh lá
#             orientation="h",
...
#             },
#             hover_data=["main_genre"]               # genre xem trong hover
#         )
...
#         # Tính số bài (unique) theo từng nghệ sĩ (đã tách collab)
#         artist_song = (
#             df[["artist", "song"]]
#             .dropna()
...
#     # =========================
#     # 🔥 Popularity Trend (so sánh nhiều bài)
#     # =========================
#     if "date" in df.columns:
#         st.subheader("🔥 Biến động độ hot theo thời gian")

#         # Lấy danh sách bài phổ biến nhất (để list không quá dài)
#         songs_available = (
#             df["song"]
#             .value_counts()
#             .head(50)   # muốn rộng hơn thì tăng lên, ví dụ 100
#             .index
#             .tolist()
#         )
...
#             default=songs_available[:1],   # mặc định 1 bài đầu
#             key=f"multi_song_{country_name}"
#         )

#         if selected_songs:
#             song_df = (
#                 df[df["song"].isin(selected_songs)]
#                 .sort_values("date")
#             )
...
#         else:
#             st.info("👉 Hãy chọn ít nhất 1 bài để hiển thị.")


#     # # ============================================================
#     # # 6) Longevity vs Peak Rank — sửa Y thành số TUẦN/NGÀY xuất hiện
#     # #    -> không còn case '0 ngày mà vẫn có rank'
#     # # ============================================================
...
#     #             periods_on_chart=("date", "nunique")  # số ngày/tuần xuất hiện
#     #         )
#     #         .reset_index()
#     #         .query("periods_on_chart > 0")
#     #     )

#     #     fig_longevity = px.scatter(
#     #         stats, x="best_rank", y="periods_on_chart",
#     #         hover_data=["song"],
#     #         color="best_rank", color_continuous_scale="Greens_r",
#     #         title="⏳ Độ Bền (số ngày/tuần xuất hiện) vs Thứ Hạng Cao Nhất",
#     #         labels={"best_rank": "Best Rank (nhỏ là tốt)", "periods_on_chart": "Số ngày/tuần xuất hiện"}
#     #     )
#     #     fig_longevity.update_xaxes(autorange="reversed")  # rank nhỏ nằm bên phải đồ thị
...
#         # Số ngày xuất hiện = (last - first) + 1
#         song_stats["days_on_chart"] = (
#             song_stats["last_date"] - song_stats["first_date"]
#         ).dt.days + 1

#         # (Tuỳ chọn) Nếu bạn muốn bỏ hẳn các bài chỉ xuất hiện 1 ngày:
#         # song_stats = song_stats[song_stats["days_on_chart"] > 1]
