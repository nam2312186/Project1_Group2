...
# Trong file config.py
    ...
    def _stream(
            ...
            **kwargs: Any,
            ...
            for attempt in range(max_retries):
                # Lấy thông tin key hiện tại để log
                current_key = self.google_api_key
                if hasattr(current_key, "get_secret_value"):
                    key_str = current_key.get_secret_value()
                else:
                ...
                try:
                    # Gọi hàm _stream của class cha
                    # Sử dụng yield from để trả về generator
                    yield from super()._stream(messages, stop, run_manager, **kwargs)
                    return # Nếu chạy xong không lỗi thì thoát vòng lặp retry

                except Exception as e:
                    error_msg = str(e)
                    short_error = error_msg.splitlines()[0] if error_msg else "Unknown Error"

                    print(f"❌ [Stream Error] Key đang dùng gặp lỗi: {short_error}")
                    ...
                    if any(code in error_msg for code in retry_codes):
                        ...
                        # Lỗi code logic thì raise luôn
                        raise e
            
            print("⛔ [Stop Stream] Đã thử hết tất cả API Key!")
            raise Exception("❌ All API Keys exhausted in Stream!")
